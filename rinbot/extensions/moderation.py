...
from rinbot.base.responder import respond, format_exception
...
class Moderation(Cog, name="moderation"):
    ...
    # Command groups
    warnings = Group(name=text['MODERATION_WARNINGS_NAME'], description=text['MODERATION_WARNINGS_DESC'])
    admins = Group(name=text['MODERATION_ADMINS_NAME'], description=text['MODERATION_ADMINS_DESC'])
    blacklist = Group(name=text['MODERATION_BL_NAME'], description=text['MODERATION_BL_DESC'])
    ...
    async def _warnings_add(self, interaction:Interaction, member:discord.Member=None, warn:str=None) -> None:
        try:
            ...
            await update_table("warns", warns)
            ...
            try:
                ...
                await respond(interaction, YELLOW, message=f"{member.mention}, {text['MODERATION_WARNINGS_PRIVATE_MSG'][0]} **{interaction.user}**!\n{text['MODERATION_WARNINGS_PRIVATE_MSG'][2]} {warn}", response_type=1)
            if len(total) >= 6:
                await member.kick(reason=f"{text['MODERATION_WARNINGS_BAN_LIMIT_REACHED']} 6.")
                await respond(interaction, RED, message=f"{member.name} {text['MODERATION_WARNINGS_WARN_LIMIT_REACHED_EMBED']}", response_type=1)
        except Exception as e:
            await respond(interaction, RED, message=f"{format_exception(e)}")
    ...
    @not_blacklisted()
    @is_admin()
    async def _warnings_remove(self, interaction:Interaction, member:discord.Member=None, warn_id:str=None) -> None:
        try:
            ...
            user_warns = guild_warns[str(member.id)]
            total = len(user_warns)
            await respond(interaction, GREEN, message=f"{text['MODERATION_WARNINGS_WARN_REMOVED'][0]} **#{warn_id}** {text['MODERATION_WARNINGS_WARN_REMOVED'][1]} **{member}**!\n{text['MODERATION_WARNINGS_WARN_REMOVED'][2]} {total}")
        except Exception as e:
            await respond(interaction, RED, message=f"{format_exception(e)}")
    ...
    async def _admins_add(self, interaction:Interaction, member:discord.Member=None) -> None:
        try:
            ...
            if str(member.id) in admins[str(interaction.guild.id)]:
                ...
                await update_table("admins", admins)
                await respond(interaction, GREEN, message=f"{text['MODERATION_ADMINS_ADDED'][0]}{member.name} {text['MODERATION_ADMINS_ADDED'][1]}")
        except Exception as e:
            await respond(interaction, RED, message=f"{format_exception(e)}")
    ...
    async def _admins_rem(self, interaction:Interaction, member:discord.Member=None) -> None:
        try:
            ...
            await update_table("admins", admins)
            await respond(interaction, GREEN, message=f"{text['MODERATION_ADMINS_REMOVED'][0]}{member.name} {text['MODERATION_ADMINS_REMOVED'][1]}")
        except Exception as e:
            await respond(interaction, RED, message=f"{format_exception(e)}")
    ...
    async def _admins_set(self, interaction:Interaction):
        if not interaction.user.guild_permissions.administrator:
            ...
            await update_table("admins", admins)
            await respond(interaction, GREEN, message=f"**{interaction.user.name}** {text['MODERATION_ADMINS_SET_ADDED']}")            

    # Shows users on the guild's blacklist
    @blacklist.command(
    ...
    async def _blacklist_show(self, interaction:Interaction) -> None:
        blacklist = await get_table("blacklist")
    ...
    async def _blacklist_add(self, interaction:Interaction, member:discord.Member=None) -> None:
        ...
        if str(member.id) in blacklist[str(interaction.guild.id)]:
            return await respond(interaction, RED, message=f"**{member.name}** {text['MODERATION_BLACKLIST_IS_BLACKLISTED']}")
        blacklist[str(interaction.guild.id)].append(str(member.id))
        await update_table("blacklist", blacklist)
    ...
    async def _blacklist_rem(self, interaction:Interaction, member:discord.Member=None) -> None:
        ...
        if not str(member.id) in blacklist[str(interaction.guild.id)]:
            return await respond(interaction, RED, message=f"**{member.name}** {text['MODERATION_BLACKLIST_NOT_ON_BLACKLIST']}")
    ...
    async def _nick(self, interaction:Interaction, member:discord.Member=None, nick:str=None) -> None:
        ...
        try:
            ...
            await respond(interaction, GREEN, message=f"{member.mention}{text['MODERATION_NICK_CHANGED'][0]} `{old}` {text['MODERATION_NICK_CHANGED'][1]} `{nick}`")
        except Exception as e:
            await respond(interaction, RED, message=f"{format_exception(e)}")

    # Delete a number of messages from the text channel
    ...
    async def _censor(self, ctx:Context, amount:str=None, from_user:discord.Member=None) -> None:
        ...
        def is_me(m): return m.author == from_user
        try:
            await respond(ctx, YELLOW, message=f"{text['MODERATION_CENSOR_EMBED'][0]} {amount} {text['MODERATION_CENSOR_EMBED'][1]} `{ctx.channel.name}`")
            if from_user: await ctx.channel.purge(limit=int(amount), check=is_me)
            else: await ctx.channel.purge(limit=int(amount))
        except Exception as e:
