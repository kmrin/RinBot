    ...
    def _get_similar_color_idx(self):
        ...
        for idx in range(1, 256):
            ...
            if color_item == old_color:
                return idx
            distance = sum((
                abs(old_color[0] - color_item[0]),   # Vermelho
                abs(old_color[1] - color_item[1]),   # Verde
                abs(old_color[2] - color_item[2])))  # Azul
...
def _create_animated_gif(images: List[Image], durations: Union[int, List[int]]) -> Tuple[Image, dict]:
    ...
    for frame in images:
        thumbnail = frame.copy()  # tipo: Image
        ...
        thumbnail_p = converter.process()  # tipo: Image
    ...
    save_kwargs.update(
        ...
        duration=durations,
        disposal=2,  # Outros disposals não funcionam
        loop=0)
